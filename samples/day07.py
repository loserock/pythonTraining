...
def sep_ipv7(addr):
    b, _t, a = str(addr).partition('[')
    normParts = [b]
    hypParts = []
    while a:
        hypSeq, _t, rest = a.partition(']')
        hypParts.append(hypSeq)
        b, _t, a = rest.partition('[')
        normParts.append(b)
    return normParts, hypParts


def get_all_aba_from_list(l):
    abas = []
    for s in l:
        for i, c in enumerate(s):
            if (i == 0) or (i >= len(s) - 1):
                continue
            if c != s[i - 1] and (s[i - 1] == s[i + 1]):
...
def test(IPs):
    n, h = sep_ipv7(IPs[1])
    print n, h
    print any([is_abba_in(i) for i in n])
    print not any([is_abba_in(i) for i in h])
    print get_all_aba_from_list(n)
    bab = lambda aba: aba[1] + aba[0] + aba[1]
    babs = [bab(i) for i in get_all_aba_from_list(n)]
    print babs
    print any((bab in h for bab in babs))
    ###########
    eg = ["aba[bab]xyz", "xyx[xyx]xyx", "aaa[kek]eke", "zazbz[bzb]cdb"]
    v = 0
    for l in eg:
        print l
        n, h = sep_ipv7(l)
        abas = get_all_aba_from_list(n)
        print abas
        if len(abas) == 0:
            continue
        skipNext = False
...
def main():
    ...
    for l in IPs:
        n, h = sep_ipv7(l)
        if any((is_abba_in(i) for i in n)) and not any((is_abba_in(i) for i in h)):
            valid += 1
    print valid
    # PART 2
    valid = 0
...
s = """dnwtsgywerfamfv[gwrhdujbiowtcirq]bjbhmuxdcasenlctwgh
